	...
	namespace Internal {
		...
		class HighlightDefinition
		{
			public:
				...
				const QHash<QString, QSharedPointer<KeywordList>> &keywords() const;
			...
			private:
				...
				GenericHelper mHelper;

				QHash<QString, QSharedPointer<KeywordList> > mLists;
				QHash<QString, QSharedPointer<Context> > mContexts;
				QHash<QString, QSharedPointer<ItemData> > mItemsData;

				QString mInitialContext;

				QString mSingleLineComment;
				bool mSingleLineCommentAfterWhiteSpaces;

				QString mMultiLineCommentStart;
				QString mMultiLineCommentEnd;
				QString mMultiLineCommentRegion;

				Qt::CaseSensitivity mKeywordCaseSensitivity;

				bool mIndentationBasedFolding;

				QSet<QChar> mDelimiters;
		};
