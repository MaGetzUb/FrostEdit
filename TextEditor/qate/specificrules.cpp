...
void RegExprRule::setInsensitive(const QString &insensitive)
{
	...
	if((opts & QRegularExpression::CaseInsensitiveOption) == QRegularExpression::CaseInsensitiveOption) {
		if(toBool(insensitive) == true)
			return;
		else
			opts = opts & ~QRegularExpression::CaseInsensitiveOption;
	} else {
		if(toBool(insensitive) == false)
			return;
		else
			opts = opts | QRegularExpression::CaseInsensitiveOption;
	}
...
{
	QRegularExpression::PatternOptions opts = m_expression.patternOptions();
	if((opts & QRegularExpression::InvertedGreedinessOption) == QRegularExpression::CaseInsensitiveOption) {
		if(toBool(minimal) == true)
			return;
		else
			opts = opts & ~QRegularExpression::InvertedGreedinessOption;
	} else {
		if(toBool(minimal) == false)
			return;
		else
			opts = opts | QRegularExpression::InvertedGreedinessOption;
	}
				 ...
				 ProgressData *progress)
{
	...
	QRegularExpressionMatch match = m_expression.match(text, offset, QRegularExpression::NormalMatch);
	if(match.capturedTexts().length() > 1) {
		m_offset = match.capturedStart(1);
		m_length = match.capturedLength(1);
		m_captures = (QStringList() << match.capturedTexts()[1]);
	} else {
		m_offset = match.capturedStart();
		m_length = match.capturedLength();
...
//void WordDetectRule::doReplaceExpressions(const QStringList& captures)
//{
