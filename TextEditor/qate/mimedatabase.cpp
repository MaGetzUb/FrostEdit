...
//#define QTC_ASSERT(_ASSERT_) Q_ASSERT(_ASSERT_);
...
*/
		...
		FileMatchContext::FileMatchContext(const QFileInfo &fi) :
			...
			mState(fi.isFile() && fi.isReadable() && fi.size() > 0 ? DataNotRead : NoDataAvailable) {
		}

		QByteArray FileMatchContext::data() {
			// Do we need to read?
			if (mState == DataNotRead) {
				const QString fullName = mFileInfo.absoluteFilePath();
				QFile file(fullName);
				if (file.open(QIODevice::ReadOnly)) {
					mData = file.read(MaxData);
					mState = DataRead;
				} else {
					qWarning("%s failed to open %s: %s\n", Q_FUNC_INFO, fullName.toUtf8().constData(), file.errorString().toUtf8().constData());
					mState = NoDataAvailable;
				}
			}
			return mData;
		}
 ...
 */
	...
	MagicRule::MagicRule(int startPos, int endPos) : mStartPos(startPos), mEndPos(endPos) {
	}
 ...
 */
	...
	MagicStringRule::MagicStringRule(const QString &s, int startPos, int endPos) :
		MagicRule(startPos, endPos), mPattern(s.toUtf8()) {
	}
	...
	bool MagicStringRule::matches(const QByteArray &data) const {
		// Quick check
		if ((startPos() + mPattern.size()) > data.size())
			return false;
		// Most common: some string at position 0:
		if (startPos() == 0 && startPos() == endPos())
			return data.startsWith(mPattern);
		// Range
		const int index = data.indexOf(mPattern, startPos());
		const bool rc = index != -1 && index <= endPos();
		if (debugMimeDB)
			qDebug() << "Checking " << mPattern << startPos() << endPos() << " returns " << rc;
		return rc;
	}
 ...
 */
	...
	MagicByteRule::MagicByteRule(const QString &s, int startPos, int endPos) :
		MagicRule(startPos, endPos), mBytesSize(0) {
		if (validateByteSequence(s, &mBytes))
			mBytesSize = mBytes.size();
		else
			mBytes.clear();
	}
	...
	bool MagicByteRule::matches(const QByteArray &data) const {
		...
		for (int start = startPos(); start <= endPos(); ++start) {
			if ((start + mBytesSize) > dataSize)
				return false;

			int matchAt = 0;
			while (matchAt < mBytesSize) {
				if (data.at(start + matchAt) != mBytes.at(matchAt))
					break;
				++matchAt;
			}
			if (matchAt == mBytesSize)
				return true;
		}
...
*/
	...
	bool MagicRuleMatcher::matches(const QByteArray &data) const {
		const MagicRuleList::const_iterator cend = mList.constEnd();
		for (MagicRuleList::const_iterator it = mList.constBegin(); it != cend; ++it)
			if ( (*it)->matches(data))
				return true;
		return false;
	}
...
*/
	...
	class MimeTypeData : public QSharedData {
		public:
			...
			const QRegExp mSuffixPattern;

			QString mType;
			QString mComment;

			LocaleHash mLocaleComments;
			QStringList mAliases;
			QList<MimeGlobPattern> mGlobPatterns;
			QStringList mSubClassesOf;
			QString mPreferredSuffix;
			QStringList mSuffixes;
			IMagicMatcher::IMagicMatcherList mMagicMatchers;
	};

	MimeTypeData::MimeTypeData()
	// RE to match a suffix glob pattern: "*.ext" (and not sth like "Makefile" or
	// "*.log[1-9]"
		: mSuffixPattern(QLatin1String("^\\*\\.[\\w+]+$")) {
		QTC_ASSERT(mSuffixPattern.isValid(), /**/);
	}
	...
	void MimeTypeData::clear() {
		...
		mGlobPatterns.clear();
		mSubClassesOf.clear();
		mPreferredSuffix.clear();
		mSuffixes.clear();
		mMagicMatchers.clear();
	}

	void MimeTypeData::assignSuffix(const QString &pattern) {
		if (mSuffixPattern.exactMatch(pattern)) {
			const QString suffix = pattern.right(pattern.size() - 2);
			mSuffixes.push_back(suffix);
			if (mPreferredSuffix.isEmpty())
				mPreferredSuffix = suffix;
		}
	}
	...
	void MimeTypeData::debug(QTextStream &str, int indent) const {
		...
		str << indentS << "Type: " << mType;
		if (!mAliases.empty())
			str << " Aliases: " << mAliases.join(comma);
		str << ", magic: " << mMagicMatchers.size() << '\n';
		str << indentS << "Comment: " << mComment << '\n';
		if (!mSubClassesOf.empty())
			str << indentS << "SubClassesOf: " << mSubClassesOf.join(comma) << '\n';
		if (!mGlobPatterns.empty()) {
			str << indentS << "Glob: ";
			foreach (const MimeGlobPattern &gp, mGlobPatterns)
				str << gp.regExp().pattern() << '(' << gp.weight() << ')';
			str << '\n';
			if (!mSuffixes.empty()) {
				str <<  indentS << "Suffixes: " << mSuffixes.join(comma)
				     << " preferred: " << mPreferredSuffix << '\n';
			}
		}
		str << '\n';
	}
	...
	QString MimeType::localeComment(const QString &localeArg) const {
		...
		const MimeTypeData::LocaleHash::const_iterator it = mD->mLocaleComments.constFind(locale);
		if (it == mD->mLocaleComments.constEnd())
			return mD->mComment;
		return it.value();
	}

	void MimeType::setLocaleComment(const QString &locale, const QString &comment) {
		mD->mLocaleComments[locale] = comment;
	}
	...
	QList<MimeGlobPattern> MimeType::globPatterns() const {
		return mD->mGlobPatterns;
	}

	void MimeType::setGlobPatterns(const QList<MimeGlobPattern> &g) {
		mD->mGlobPatterns = g;

		QString oldPrefferedSuffix = mD->mPreferredSuffix;
		mD->mSuffixes.clear();
		mD->mPreferredSuffix.clear();
		mD->assignSuffixes(MimeDatabase::fromGlobPatterns(g));
		if (mD->mPreferredSuffix != oldPrefferedSuffix && mD->mSuffixes.contains(oldPrefferedSuffix))
			mD->mPreferredSuffix = oldPrefferedSuffix;
	}

	QStringList MimeType::subClassesOf() const {
		return mD->mSubClassesOf;
	}
	...
	QString MimeType::preferredSuffix() const {
		return mD->mPreferredSuffix;
	}

	bool MimeType::setPreferredSuffix(const QString &s) {
		if (!mD->mSuffixes.contains(s)) {
			qWarning("%s: Attempt to set preferred suffix to '%s', which is not in the list of suffixes: %s.",
				 mD->mType.toUtf8().constData(),
				 s.toUtf8().constData(),
				 mD->mSuffixes.join(QLatin1String(",")).toUtf8().constData());
			return false;
		}
		mD->mPreferredSuffix = s;
		return true;
	}
	...
	QString MimeType::filterString() const {
		...
		return formatFilterString(comment(), mD->mGlobPatterns);
	}

	bool MimeType::matchesType(const QString &type) const {
		return mD->mType == type || mD->mAliases.contains(type);
	}
	...
	unsigned MimeType::matchesFileBySuffix(Internal::FileMatchContext &c) const {
		// check globs
		foreach (const MimeGlobPattern &gp, mD->mGlobPatterns) {
	...
	unsigned MimeType::matchesData(const QByteArray &data) const {
		unsigned priority = 0;
		if (!data.isEmpty()) {
			foreach (const IMagicMatcher::IMagicMatcherSharedPointer &matcher, mD->mMagicMatchers) {
	...
	MimeType::IMagicMatcherList MimeType::magicRuleMatchers() const {
		IMagicMatcherList ruleMatchers = mD->mMagicMatchers;
	...
	void MimeType::setMagicRuleMatchers(const IMagicMatcherList &matchers) {
		mD->mMagicMatchers.erase(std::remove_if(mD->mMagicMatchers.begin(), mD->mMagicMatchers.end(),
							RemovePred(false)),
					 mD->mMagicMatchers.end());
		mD->mMagicMatchers.append(matchers);
	}
...
*/
		...
		void BaseMimeTypeParser::addGlobPattern(const QString &pattern, const QString &weight, MimeTypeData *d) const {
			...
			if (weight.isEmpty())
				d->mGlobPatterns.push_back(MimeGlobPattern(wildCard));
			else
				d->mGlobPatterns.push_back(MimeGlobPattern(wildCard, weight.toInt()));
				...
				case QXmlStreamReader::StartElement:
					...
					case ParseSubClass: {
						...
						if (!inheritsFrom.isEmpty())
							data.mSubClassesOf.push_back(inheritsFrom);
					}
					break;
					case ParseComment: {
						...
						if (locale.isEmpty()) {
							...
							data.mLocaleComments.insert(locale, comment);
						}
					}
					break;
					case ParseAlias: {
				...
				case QXmlStreamReader::EndElement: // Finished element
					if (reader.name() == QLatin1String(mimeTypeTagC)) {
						...
						if (reader.name() == QLatin1String(magicTagC)) {
							QTC_ASSERT(!ruleMatcher.isNull(), return false)
									data.mMagicMatchers.push_back(ruleMatcher);
...
*/
	...
	void MimeDatabasePrivate::debug(QTextStream &str) const {
		...
		for (TypeMimeTypeMap::const_iterator it = m_typeMimeTypeMap.constBegin(); it != cend; ++it) {
			...
			it.value().type.mD->debug(str);
		}
		str << "<MimeDatabase\n";
	}

	MimeDatabase::MimeDatabase() :
		mD(new MimeDatabasePrivate) {
	}
	...
	MimeType MimeDatabase::findByType(const QString &typeOrAlias) const {
		mMutex.lock();
		const MimeType rc = mD->findByType(typeOrAlias);
		mMutex.unlock();
		return rc;
	}

	MimeType MimeDatabase::findByFileUnlocked(const QFileInfo &f) const {
		return mD->findByFile(f);
	}
	...
	MimeType MimeDatabase::findByData(const QByteArray &data) const {
		mMutex.lock();
		const MimeType rc = mD->findByData(data);
		mMutex.unlock();
		return rc;
	}
	...
	bool MimeDatabase::addMimeType(const  MimeType &mt) {
		mMutex.lock();
		const bool rc = mD->addMimeType(mt);
		mMutex.unlock();
		return rc;
	}
	...
	bool MimeDatabase::addMimeTypes(const QString &fileName, QString *errorMessage) {
		mMutex.lock();
		const bool rc = mD->addMimeTypes(fileName, errorMessage);
		mMutex.unlock();
		return rc;
	}
	...
	bool MimeDatabase::addMimeTypes(QIODevice *device, QString *errorMessage) {
		mMutex.lock();
		const bool rc = mD->addMimeTypes(device, errorMessage);
		mMutex.unlock();
		return rc;
	}
	...
	QStringList MimeDatabase::suffixes() const {
		mMutex.lock();
		const QStringList rc = mD->suffixes();
		mMutex.unlock();
		return rc;
	}
	...
	QStringList MimeDatabase::filterStrings() const {
		mMutex.lock();
		const QStringList rc = mD->filterStrings();
		mMutex.unlock();
		return rc;
	}
	...
	QList<MimeGlobPattern> MimeDatabase::globPatterns() const {
		mMutex.lock();
		const QList<MimeGlobPattern> rc = mD->globPatterns();
		mMutex.unlock();
		return rc;
	}
	...
	void MimeDatabase::setGlobPatterns(const QString &typeOrAlias,
					   const QList<MimeGlobPattern> &globPatterns) {
		mMutex.lock();
		mD->setGlobPatterns(typeOrAlias, globPatterns);
		mMutex.unlock();
	}

	MimeDatabase::IMagicMatcherList MimeDatabase::magicMatchers() const {
		mMutex.lock();
		const IMagicMatcherList rc = mD->magicMatchers();
		mMutex.unlock();
		return rc;
	}
	...
	void MimeDatabase::setMagicMatchers(const QString &typeOrAlias,
		...
		mMutex.lock();
		mD->setMagicMatchers(typeOrAlias, matchers);
		mMutex.unlock();
	}

	QList<MimeType> MimeDatabase::mimeTypes() const {
		mMutex.lock();
		const QList<MimeType> &mimeTypes = mD->mimeTypes();
		mMutex.unlock();
		return mimeTypes;
	}
	...
	void MimeDatabase::syncUserModifiedMimeTypes() {
		mMutex.lock();
		mD->syncUserModifiedMimeTypes();
		mMutex.unlock();
	}

	void MimeDatabase::clearUserModifiedMimeTypes() {
		mMutex.lock();
		mD->clearUserModifiedMimeTypes();
		mMutex.unlock();
	}
	...
	bool MimeDatabase::setPreferredSuffix(const QString &typeOrAlias, const QString &suffix) {
		mMutex.lock();
		const bool rc = mD->setPreferredSuffix(typeOrAlias, suffix);
		mMutex.unlock();
		return rc;
	}
