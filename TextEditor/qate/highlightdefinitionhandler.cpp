...
HighlightDefinitionHandler(const QSharedPointer<HighlightDefinition> &definition) :
	...
	mProcessingKeyword(false),
	mInitialContext(true)
{}
...
bool HighlightDefinitionHandler::endElement(const QString &, const QString &, const QString &qName) {
	if (qName == kItem) {
		mCurrentList->addKeyword(mCurrentKeyword.trimmed());
		mProcessingKeyword = false;
...
bool HighlightDefinitionHandler::characters(const QString& ch) {
	...
	if (mProcessingKeyword)
		mCurrentKeyword.append(ch);

	return true;
}

void HighlightDefinitionHandler::listElementStarted(const QXmlAttributes &atts) {
	mCurrentList = m_definition->createKeywordList(atts.value(kName));
}

void HighlightDefinitionHandler::itemElementStarted() {
	mCurrentKeyword.clear();
	mProcessingKeyword = true;
}

void HighlightDefinitionHandler::contextElementStarted(const QXmlAttributes &atts) {
	mCurrentContext = m_definition->createContext(atts.value(kName), mInitialContext);
	mCurrentContext->setDefinition(m_definition);
	mCurrentContext->setItemData(atts.value(kAttribute));
	mCurrentContext->setDynamic(atts.value(kDynamic));
	mCurrentContext->setFallthrough(atts.value(kFallthrough));
	mCurrentContext->setFallthroughContext(atts.value(kFallthroughContext));
	mCurrentContext->setLineBeginContext(atts.value(kLineBeginContext));
	mCurrentContext->setLineEndContext(atts.value(kLineEndContext));

	mInitialContext = false;
}

void HighlightDefinitionHandler::ruleElementStarted(const QXmlAttributes &atts, const QSharedPointer<Rule> &rule) {
	...
	if (mCurrentRule.isEmpty())
		mCurrentContext->addRule(rule);
	else
		mCurrentRule.top()->addChild(rule);

	mCurrentRule.push(rule);
}
...
void HighlightDefinitionHandler::includeRulesStarted(const QXmlAttributes &atts) {
	...
	IncludeRulesInstruction instruction(atts.value(kContext), mCurrentContext->rules().size(),
					    atts.value(kIncludeAttrib));

	// Include rules (as many others) are not allowed as a child.
	mCurrentContext->addIncludeRulesInstruction(instruction);
}
...
void HighlightDefinitionHandler::processIncludeRules(const QSharedPointer<Context> &context) const {
	...
	for(const IncludeRulesInstruction &instruction: instructions) {
		...
		for(QSharedPointer<Rule> rule: sourceContext->rules()) {
